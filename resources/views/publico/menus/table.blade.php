        ...
        @foreach ($menuspadre as $menu)
            <tr>
                ...
                <td>
                    ...
                    @can('eliminar-menu')
                        <div class='btn-group'>
                            {{ html()->form('DELETE')->route('menus.destroy', [$menu->id])->class('delete-form')->open() }}
                            <button type="submit" class="btn btn-sm btn-danger" id="eliminar"
                                data-mensaje="el menú {{ $menu->name }}">
        ...
        @foreach ($menushijos as $menuh)
            <tr>
                ...
                <td>
                    ...
                    @can('eliminar-menu')
                        <div class='btn-group'>
                            {{ html()->form('DELETE')->route('menus.destroy', [$menuh->id])->class('delete-form')->open() }}
                            <button type="submit" class="btn btn-sm btn-danger" id="eliminar"
                                data-mensaje="el menú {{ $menuh->name }}">
                                <i class="fa fa-trash"></i></button>
                            {{ html()->form()->close() }}
                        </div>
                    @endcan
